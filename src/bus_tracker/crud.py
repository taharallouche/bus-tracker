...
def get_bus(db: Session, line_number: int) -> models.Bus:
...
def get_log(db: Session, log_id: int) -> models.Log:
...
def get_logs_by_bus(db: Session, line_number: int, skip: int = 0, limit: int = 100):
    return (
        db.query(models.Log)
